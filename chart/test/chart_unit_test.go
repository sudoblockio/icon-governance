...
func TestIconBlocksUnit(t *testing.T) {
	...
	apiOutput := helm.RenderTemplate(t, options, helmChartPath, "deployment-icon-governance*", []string{"templates/deployment-api.yaml"})
	workerOutput := helm.RenderTemplate(t, options, helmChartPath, "deployment-icon-governance*", []string{"templates/deployment-worker.yaml"})
